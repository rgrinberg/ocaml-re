...
let rec deriv_1 all_chars categories marks cat x rem =
  match x.def with
    Cst s ->
      prepend s [E.texp marks eps_expr] rem
  | Alt l ->
  ...
  | Rep (rep_kind, kind, y) ->
      ...
      List.fold_right
        ...
        y' rem
  | Eps ->
      prepend all_chars [E.TMatch marks] rem
  | Mark i ->
      prepend all_chars [E.TMatch {marks with marks = ((i, -1) :: List.remove_assq i marks.marks)}] rem
  | Pmark _ ->
      prepend all_chars [E.TMatch marks] rem
  | Erase (b, e) ->
      prepend all_chars
  ...
  | Before cat' ->
      prepend (List.assq cat' categories) [E.TMatch marks] rem
  | After cat' ->
      if cat land cat' <> 0 then prepend all_chars [E.TMatch marks] rem else rem
  ...
  then
    ...
    List.fold_right
      (fun (s, y) rem ->
         ...
         with
           None ->
             prepend s (E.tseq kind y z []) rem
         | Some marks ->
             let z'' = prepend_marks marks z' in
             match kind with
               `Longest ->
                 prepend s (E.tseq kind (remove_matches y) z []) (
                 prepend_deriv (restrict s z'') rem)
             | `Shortest ->
                 prepend_deriv (restrict s z'') (
                 prepend s (E.tseq kind (remove_matches y) z []) rem)
             | `First ->
                 let (y', y'') = split_at_match y in
                 prepend s (E.tseq kind y' z []) (
                 prepend_deriv (restrict s z'') (
                 prepend s (E.tseq kind y'' z []) rem)))
      y rem
  else
    List.fold_right
      (fun (s, xl) rem -> prepend s (E.tseq kind xl z []) rem) y rem
